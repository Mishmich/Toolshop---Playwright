...
test.describe("Products API", () => {
  ...
  test("GET /api/products - Retrieve all products with pagination", async ({
    request,
  }) => {
  ...
  test("GET /api/products/:id - Get single product details", async ({
    request,
  }) => {
    const list = await request.get(`${API_BASE_URL}/products`);
    const firstId = (await list.json()).data[0].id;
    const response = await request.get(`${API_BASE_URL}/products/${firstId}`);
  ...
  test("GET /api/products - Filter products by category", async ({
    request,
  }) => {
